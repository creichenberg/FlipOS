    ...
    >
      <div>
        <div className="flex items-center justify-between gap-2">
          <span className="font-mono text-xs uppercase tracking-wide text-text-secondary">{DAY_LABELS[card.day_of_week]}</span>
      ...
      <div className="mt-4 flex items-center justify-between gap-2">
        <StatusBadge label={STATUS_LABELS[card.status] ?? card.status} variant={card.status === 'complete' ? 'success' : 'neutral'} />
